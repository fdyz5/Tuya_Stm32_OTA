...
//    #error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"
   ...
   // mcu_dp_enum_update(DPID_LIGHT_MODE,当前指示灯状态设置); //枚举型数据上报;
}
...
/*****************************************************************************
函数名称 : dp_download_countdown_1_handle
功能描述 : 针对DPID_COUNTDOWN_1的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
...
void upgrade_package_choose(unsigned char package_sz)
{
	...
	send_len = set_wifi_uart_byte(send_len, package_sz);//写wifi_uart字节
	wifi_uart_write_frame(UPDATE_START_CMD, MCU_TX_VER, send_len);//向串口发送
}
/**
 * @brief  MCU进入固件升级模式
...
unsigned char mcu_firm_update_handle(const unsigned char value[],unsigned long position,unsigned short length)
{
    ...
    if(length == 0)  {  
			...
			//固件数据发送完成 先置升级标志位 然后复位系统
			EarseFlash_1K(UPFLAGADDR);//清除系统标志位地址数据
			WriteFlash(UPFLAGADDR,nbuff_5555,4/4);//置系统升级标志位
			GPIO_SetBits(GPIOC,GPIO_Pin_13);

			NVIC_SystemReset();//进行系统复位
			//更新完毕无需定时器中断
    }else {
			//固件数据为1024B时进行一次擦除处理，其他正常写入
			if(position %1024 == 0)
			{		
				EarseFlash_1K(APP2ADDR+position);//擦除指定地址的一页
			}
			WriteFlash(APP2ADDR+position,(uint8_t *)value,length);//写入指定的数据
    }
    return SUCCESS;
}
#endif
